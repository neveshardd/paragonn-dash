<script lang="ts">
	...
	async function onFile(ev: Event) {
		...
		uploading = true;
		try {
			const presign = await fetch('/api/uploads/blog', {
				method: 'POST',
...
<div class="blog-imagem-field">
	<div class="field">
		<label for="imagem-url">URL da imagem (capa do post)</label>
		<input
			id="imagem-url"
			name="imagem"
			type="url"
	...
	{#if value}
		<div class="preview">
			<img src={value} alt="Pré-visualização da capa" referrerpolicy="no-referrer" />
		</div>
	{/if}
</div>
