...
import { t as ProdutoImagemField } from "../../../../../chunks/ProdutoImagemField.js";
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		function $$render_inner($$renderer) {
			...
			$$renderer.push(`<!--]--></select></div></div> <div class="field"><label for="descricao">Descrição</label> <textarea id="descricao" name="descricao" placeholder="Descrição do produto (opcional)"></textarea></div> <div class="field">`);
			ProdutoImagemField($$renderer, {
				get value() {
					return imagem;
				},
				set value($$value) {
			...
			$$renderer.push(`<!----></div> <div class="field"><label for="comando">Comando de Entrega</label> <input id="comando" name="comando" type="text" placeholder="ex: lp user %player% parent add vip" value="lp user %player% parent add vip" required=""/> <span class="field-hint">Use %player% para o nick do jogador.</span></div> <div class="check-row"><input id="ativo" name="ativo" type="checkbox" checked=""/> <label for="ativo">Produto ativo</label></div></div> <div class="form-foot"><button type="submit" class="btn btn-primary">Criar Produto</button> <a href="/loja/produtos" class="btn">Cancelar</a></div></div></form></div>`);
		}
		do {
			$$settled = true;
